    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            if self.use_batchnorm and i < self.num_layers-1:
                self.params[gamma_list[i]] = np.ones(dims[i+1])
                self.params[beta_list[i]] = np.zeros(dims[i+1])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.use_batchnorm and self.use_dropout:
                layer[i], cache_layer[i] = affine_relu_dropout_norm_forward(layer[i - 1], self.params['W%d' % i], self.params['b%d' % i], self.params['gamma%d' % i], self.params['beta%d' % i], self.bn_params[i - 1], self.dropout_param)
            elif self.use_batchnorm and not self.use_dropout:
                layer[i], cache_layer[i] = affine_relu_norm_forward(layer[i - 1], self.params['W%d' % i], self.params['b%d' % i], self.params['gamma%d' % i], self.params['beta%d' % i], self.bn_params[i - 1])
            else:
        ...
        for i in reversed(range(1, self.num_layers)):
            if self.use_batchnorm and not self.use_dropout:
                dx[i], grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads['beta%d' % i] = affine_relu_norm_backward(dx[i + 1], cache_layer[i])
                grads['W%d' % i] += self.reg * self.params['W%d' % i]
            elif self.use_batchnorm and self.use_dropout:
                dx[i], grads['W%d' % i], grads['b%d' % i], grads['gamma%d' % i], grads['beta%d' % i] = affine_relu_dropout_norm_backward(dx[i + 1], cache_layer[i])
                grads['W%d' % i] += self.reg * self.params['W%d' % i]
            else:
